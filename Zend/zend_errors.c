...
ZEND_API void zend_error_bailout_cb(ZEND_ERROR_CB_HOOK_ARGS)
{
...
ZEND_API void zend_add_error_hook(zend_error_cb_hook_t hook_part, void (*hook)(ZEND_ERROR_CB_HOOK_ARGS))
{
...
{
	zend_add_error_hook(E_HOOK_DISPLAY, &zend_error_display_cb);
	zend_add_error_hook(E_HOOK_LOG, &zend_error_log_cb);
	zend_add_error_hook(E_HOOK_BAILOUT, &zend_error_bailout_cb);
}
